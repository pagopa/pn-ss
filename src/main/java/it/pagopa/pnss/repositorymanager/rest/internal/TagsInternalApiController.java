...
public class TagsInternalApiController implements TagsInternalApi {
    ...
    public Mono<ResponseEntity<TagsResponse>> getTags(String tagKeyValue, final ServerWebExchange exchange) {
        ...
        return tagsService.getTags(tagKeyValue)
    ...
    @Override
    public Mono<ResponseEntity<TagsUpdateResponse>> putTags(String documentKey, Mono<TagsChanges> tagsChanges, ServerWebExchange exchange) {
        return tagsChanges.flatMap(changes -> tagsService.updateTags(documentKey, changes))
                .map(result -> ResponseEntity.ok().body(new TagsUpdateResponse().tags(result)))
