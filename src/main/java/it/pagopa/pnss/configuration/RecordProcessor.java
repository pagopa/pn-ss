...
public class RecordProcessor implements ShardRecordProcessor {
    ...
    public void processRecords(ProcessRecordsInput processRecordsInput) {
        shardMap.put(SHARD_KEY, shardId);
        try {
            log.debug("Thread Name:" + Thread.currentThread().getName() + " Processing {} record(s)", processRecordsInput.records().size());
            processRecordsInput.records().forEach(r -> log.info("Processing record: {} -- Seq: {} -- Data: {}", r.partitionKey(), r.sequenceNumber(), r.data()));
        } catch (Throwable t) {
